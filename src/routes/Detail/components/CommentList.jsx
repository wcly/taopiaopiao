import React from 'react'
import PropTypes from 'prop-types'
import './CommentList.scss'
import Star from './../../../components/Star'
import ColpassibleText from './../../../components/ColpassibleText'

export default function CommentList({ data, onClickZan }) {
    return (
        <ul className="commentList">
            {
                data.map(comment => (
                    <li key={comment.id}>
                        <div className="commentItem">
                            <div className="commentUser">
                                <div className="commentUser__avatar" style={{ background: 'url(/source/default-avatar.jpg)' }}></div>
                                <div className="commentUser__detail">
                                    <div className="commentUser__name">{comment.name}</div>
                                    <div className="commentUser__score">
                                        <Star value={5} /><div className="commentUser__value">{comment.score}</div>
                                    </div>
                                </div>
                            </div>
                            <ColpassibleText>{comment.content}</ColpassibleText>
                            <div className="commentItem__detail">
                                <div className="commentItem__time">{comment.time}</div>
                                <div className={`commentItem__zan ${comment.isZan && 'commentItem__zan--active'}`}>
                                    <i onClick={() => onClickZan(comment.id)} /> {comment.zan}
                                </div>
                            </div>
                        </div>
                    </li>
                ))
            }
        </ul>
    )
}

CommentList.propTypes = {
    data: PropTypes.array.isRequired,
    onClickZan: PropTypes.func.isRequired,
}
